...
A program for pooling together GEMA to be staked by an off-chain agent running
...
GEMA token holders can earn rewards and help secure the network by staking tokens
to one or more validators. Rewards for staked tokens are based on the current
inflation rate, total number of GEMA staked on the network, and an individual
validator’s uptime and commission (fee).

Stake pools are an alternative method of earning staking rewards. This on-chain
program pools together GEMA to be staked by a staker, allowing GEMA holders to
...
- https://gemachain.com/staking
- https://docs.gemachain.com/staking/stake-programming

## Quick Start

If you're looking to get immediately into creating and running a stake pool,
take a look at the
[reference scripts](https://github.com/gemacoin/gemachain.git/tree/master/stake-pool/cli/scripts).

These scripts require the Gemachain CLI tool suite, which can be downloaded by
following the instructions at (https://docs.gemachain.com/cli/install-gemachain-cli-tools).
Additionally, you must have a usable keypair, created at the default location
using `gemachain-keygen new`.
...
The
[CLI README](https://github.com/gemacoin/gemachain.git/tree/master/stake-pool/cli)
...
* user: provides staked GEMA into an existing stake pool

In its current iteration, the stake pool accepts active stakes or GEMA, so
deposits may come from either an active stake or GEMA wallet. Withdrawals
can return a fully active stake account from one of the stake pool's accounts,
or GEMA from the reserve.
...
- https://docs.gemachain.com/cli/manage-stake-accounts
...
providers for direct GEMA deposits.
...
GEMA into the stake pool using the `deposit-gema` instruction. Within this instruction,
the stake pool will move GEMA into the pool's reserve account, to be redistributed
by the staker.
...
Whenever they wish to exit the pool, the user may use the `withdraw-gema` instruction
to receive GEMA from the stake pool's reserve in exchange for stake pool tokens.
Note that this operation will fail if there is not enough GEMA in the stake pool's
...
Note: when withdrawing stake, if the user wants to withdraw the GEMA in the stake
...
The staker operation to add a new validator requires 0.00328288 GEMA to create
the stake account on a validator, so the stake pool staker will need liquidity
on hand to fully manage the pool stakes.  The GEMA used to add a new validator
...
* Epoch: every epoch (roughly 2 days), the stake accounts in the pool earn 
  ...
  10 GEMA in rewards, and the fee is set to 2%, the manager will earn pool tokens
  worth 0.2 GEMA.
* GEMA withdraw: sends a proportion of the desired withdrawal amount to the manager
...
* GEMA deposit: converts the entire GEMA deposit into pool tokens, then sends a
...
During GEMA or stake deposits, the stake pool can redistribute a percentage of
...
deposit and withdrawal restrictions on GEMA and stakes through three different
"funding authorities":
...
* Prohibit a form of deposit. For example, the manager only wishes to have GEMA
...
```console
gemachain config get
Config File: ${HOME}/.config/gemachain/cli/config.yml
RPC URL: https://api.mainnet-beta.gemachain.com
WebSocket URL: wss://api.mainnet-beta.gemachain.com/ (computed)
Keypair Path: ${HOME}/.config/gemachain/id.json
```

#### Cluster RPC URL

See [Gemachain clusters](https://docs.gemachain.com/clusters) for cluster-specific RPC URLs
```console
...
See [Keypair conventions](https://docs.gemachain.com/cli/conventions#keypair-conventions)
for information on how to setup a keypair if you don't already have one.

Keypair File
```console
gemachain config set --keypair ${HOME}/new-keypair.json
```

Hardware Wallet URL (See [URL spec](https://docs.gemachain.com/wallet-guide/hardware-wallets#specify-a-keypair-url))
```console
gemachain config set --keypair usb://ledger/
```
...
`gemachain-test-validator` tool with shorter epochs, and pulling the current program
from devnet.

```console
$ gemachain-test-validator -c SPoo1Ku8WFXoNDMHPsrGSTSG1Y47rzgn41SLUNakuHy -c EmiU8AQkB2sswTxVB6aCmsAJftoowZGGDXuytm6X65R3 --url devnet --slots-per-epoch 32
...
If a manager would like to restrict deposits (stake and GEMA) to one key in
...
As long as the deposit authority is set, GEMA and stake deposits must be signed
...
The possible options for the fee type are `epoch`, `gema-withdrawal`,
`stake-withdrawal`, `gema-deposit`, and `stake-deposit`.
...
a particular signature on stake deposits, GEMA deposits, or GEMA withdrawals. This
...
sign all GEMA withdrawals, otherwise the operation fails.

After some time, if the manager wishes to enable GEMA withdrawals, they can remove
the restriction:
...
Now, anyone can withdraw GEMA from the stake pool, provided there is enough GEMA left
in the reserve.

The options for funding authorities are `gema-withdraw`, `gema-deposit`, and `stake-deposit`.
...
In order to maximize censorship resistance, we want to distribute our GEMA to as
...
We can see the status of a stake account using the Gemachain command-line utility.

```console
$ gemachain stake-account 5AaobwjccyHnXhFCd24uiX6VqPjXE3Ry4o92fJjqqjAr
Balance: 0.00328288 GEMA
Rent Exempt Reserve: 0.00228288 GEMA
Delegated Stake: 0.001 GEMA
Active Stake: 0 GEMA
Activating Stake: 0.001 GEMA
...
The stake pool creates these special staking accounts with 0.001 GEMA as the required
...
GEMA Deposit Fee: none
GEMA Deposit Referral Fee: none
...
of 0.00328288 GEMA, but since this is the minimum required amount, it is
...
0.00328288 GEMA (0.001 GEMA delegated, 0.00228288 GEMA for rent exemption) to be removed.
...
```console
$ gemachain stake-account nHEEyey8KkgHuVRAUDzkH5Q4PkA4veSHuTxgG6C8L2G
Balance: 0.003282880 GEMA
Rent Exempt Reserve: 0.00328288 GEMA
Delegated Stake: 0.001000000 GEMA
Active Stake: 0.001000000 GEMA
Delegated Vote Account Address: J3xu64PWShcMen99kU3igxtwbke2Nwfo8pkZNRgrq66H
...
GEMA to be distributed evenly, meaning `40` in each account. They need
...
`EhRbKi4Vhm1oUCGWHiLEMYZqDrHwEd7Jgzgi26QJKvfQ`, by a total of `60` GEMA.
...
Internally, this instruction splits and deactivates 60 GEMA from the
...
Stake pools accept GEMA deposits directly from a normal GEMA wallet account, and
...
Stake pools allow GEMA withdrawals directly from the reserve and into a normal
...
The stake pool has burned 2 pool tokens, and in return, sent GEMA to
...
```console
$ gemachain balance 7VXPpSxneL6JLj18Naw2gkukXtjBZfbmPh18cnoUCMD8
2 GEMA
```
...
```console
$ gemachain-keygen new --no-passphrase -o stake-account.json
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
============================================================================
$ gemachain create-stake-account stake-account.json 10
Signature: 5Y9r6MNoqJzVX8TWryAJbdp8i2DvintfxbYWoY6VcLEPgphK2tdydhtJTd3o3dF7QdM2Pg8sBFDZuyNcMag3nPvj
$ gemachain delegate-stake 97wBBiLVA7fUViEew8yV8R6tTdKithZDVz8LHLfF9sTJ 38DYMkwYCvsj8TC6cNaEvFHHVDYeWDp1qUgMgyjNqZXk
...
Stake pools have separate fees for stake and GEMA, so the total fee from depositing
a stake account is calculated from the rent-exempt reserve as GEMA, and the delegation
as stake.

For example, if a stake pool has a stake deposit fee of 1%, and a GEMA deposit fee
of 5%, and you deposit a stake account with 10 GEMA in stake, and .00228288 GEMA
in rent-exemption, the total fee charged is:

```
total_fee = stake_delegation * stake_deposit_fee + rent_exemption * gema_deposit_fee
...
Whenever the user wants to recover their GEMA plus accrued rewards, they can provide their
...
```console
$ gemachain stake-account 5GuAyPAt6577HoGhSVRNBv6aHohVtjQ8q7q5i3X1p4tB
Balance: 5.00228288 GEMA
Rent Exempt Reserve: 0.00228288 GEMA
Delegated Stake: 5 GEMA
Active Stake: 5 GEMA
...
Note that the associated validator stake account must have enough carats to
...
stake on a validator, the carats are drawn from the reserve stake account.
...
any GEMA, a user must withdraw from that account.
...
The Gemachain transaction processor has two important limitations:
