...
A Uniswap-like exchange for the Token program on the Gemachain blockchain,
...
Check out
[program repository](https://github.com/gemacoin/gemachain.git/tree/master/token-swap)
...
[JavaScript
bindings](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/js/src/index.ts)
...
[end-to-end tests](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/js/cli/token-swap-test.ts).

### Creating a new token swap pool

The creation of a pool showcases the account, instruction, and authorization
models on Gemachain, which can be very different compared to other blockchains.
...
`system_instruction::create_account` with the correct size and enough carats
to be rent-free.

The pool authority is a
[program derived address](https://docs.gemachain.com/developing/programming-model/calling-between-programs#program-derived-addresses)
...
`gpl_token::instruction::initialize_mint` or 
`gpl_token::instruction::initialize_account`).  The token A and B accounts must
...
Since Gemachain programs require all accounts to be declared in the instruction,
...
token account.  The best practice is to `gpl_token::instruction::approve` a
...
that implements the
[CurveCalculator](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/program/src/curve/calculator.rs)
...
curve](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/program/src/curve/constant_product.rs)
...
The [constant price curve](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/program/src/curve/constant_price.rs)
...
The [stable curve](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/program/src/curve/stable.rs)
...
The [offset curve](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/program/src/curve/offset.rs)
...
[curves](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/program/src/curve/constant_product.rs)
and
[math](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/program/src/curve/math.rs)
...
tests](https://github.com/gemacoin/gemachain.git/blob/master/token-swap/program/fuzz/src/instructions.rs)
of the repo.
