...
fn command_deposit_gema(
    ...
    let deposit_instruction = if let Some(deposit_authority) = config.funding_authority.as_ref() {
        ...
        spl_stake_pool::instruction::deposit_gema_with_authority(
            &spl_stake_pool::id(),
            stake_pool_address,
            &deposit_authority.pubkey(),
            &pool_withdraw_authority,
        ...
        spl_stake_pool::instruction::deposit_gema(
            &spl_stake_pool::id(),
            stake_pool_address,
            &pool_withdraw_authority,
            &stake_pool.reserve_stake,
...
fn prepare_withdraw_accounts(
    ...
    accounts.push((stake_pool.reserve_stake, reserve_stake.carats, None));
    ...
    for (stake_address, carats, vote_address_opt) in accounts {
...
fn command_withdraw_gema(
    ...
    let withdraw_instruction = if let Some(withdraw_authority) = config.funding_authority.as_ref() {
        ...
        spl_stake_pool::instruction::withdraw_gema_with_authority(
            &spl_stake_pool::id(),
            stake_pool_address,
            &withdraw_authority.pubkey(),
            &pool_withdraw_authority,
        ...
        spl_stake_pool::instruction::withdraw_gema(
            &spl_stake_pool::id(),
            stake_pool_address,
            &pool_withdraw_authority,
            &user_transfer_authority.pubkey(),
    ...
    let _ = match matches.subcommand() {
        ...
        ("set-funding-authority", Some(arg_matches)) => {
            ...
            let funding_type = match arg_matches.value_of("funding_type").unwrap() {
                "gema-deposit" => FundingType::SolDeposit,
                "stake-deposit" => FundingType::StakeDeposit,
                "gema-withdraw" => FundingType::SolWithdraw,
                _ => unreachable!(),
            };
            let _unset = arg_matches.is_present("unset");
