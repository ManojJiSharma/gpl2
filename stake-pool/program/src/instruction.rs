...
pub enum StakePoolInstruction {
    ...
    ///   `crate::MINIMUM_ACTIVE_STAKE` (currently 0.001 GEMA).
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[s]` Staker
    ...
    ///   `crate::MINIMUM_ACTIVE_STAKE` (currently 0.001 GEMA) plus the rent-exempt
    ///   amount.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[s]` Staker
    ...
    ///   7. `[w]` Destination stake account, to receive the minimum GEMA from the validator stake account
    ///   8. `[]` Sysvar clock
    ///   9. `[]` Stake program id,
    ...
    /// (currently 0.001 GEMA) in order to avoid issues on credits observed when
    ...
    ///   above the minimum of rent-exempt amount + 0.001 GEMA.
    ///
